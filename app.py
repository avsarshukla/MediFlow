...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        if "metformin" in text_lower:
            st.session_state.form_data["medications"] = "Metformin detected"
...
with tab3:
    ...
    if password == "1234":
        ...
        with feed_col1:
            st.write("**🎙️ Audio Transcript:**")
            if st.session_state.audio_transcript:
                st.info(st.session_state.audio_transcript)
            else:
                st.caption("No audio recorded.")
        with feed_col2:
            st.write("**📄 Extracted Document/OCR Text:**")
            if st.session_state.ocr_extracted_text:
                st.text_area("Extracted Text", value=st.session_state.ocr_extracted_text, height=100, disabled=True)
            else:
                st.caption("No document parsed.")
